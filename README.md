...
- Проверяет несколько каналов параллельно, но держит не больше одной recording-сессии на канал;
  финализация предыдущей записи может идти параллельно с новой.
...
- При естественном завершении эфира сразу отправляет уведомление и запускает обработку MP4
  в фоне; тот же канал может начать новую запись до окончания обработки.
- После завершения обработки отправляет итоговое уведомление с результатом и метриками.
